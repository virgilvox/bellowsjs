...
During a render, \`b.rng(label)\` hands your callbacks fresh streams seeded exactly as they were at boot. Capturing the stream once outside the callback, \`const melody = b.rng('melody')\`, is fine and is what the examples do: what \`b.rng\` returns is a stable handle that resolves the right stream for whichever context is drawing. So a render equals what a fresh page load of the same seed would play, provided three things are true: every random choice flows through \`b.rng()\` (never \`Math.random\`), any other mutable state in your callbacks is derived from \`step\` or reset at a known step, and a given stream is drawn by one subscription rather than shared between two. That last one is narrow and worth stating plainly: the replay walks every tick in time order while live playback walks them one subscription at a time, so a stream shared across two subdivisions can hand a value to a different callback than the replay does. It is live playback that is the unreliable half here, not the render: if the tab is backgrounded, the browser throttles the timer, more ticks arrive per wake and the order shifts, so the same seed can play differently. One stream per part, which is what the examples do, never runs into any of it. A counter you increment across calls carries its live value into the render; a value computed from \`step\` and a named stream cannot drift. The [seeded piece example](/docs/generative-music) is built to this rule.
