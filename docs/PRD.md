...
Tracked in docs/ENGINEERING.md section 5. The live ones: Safari (no Web MIDI, late WebCodecs, sample-rate changes after interruptions), blob URL CSP failures (mitigated by URL override), constant-transcription errors in the Dattorro lengths and taps and in the BLEP residual tables (mitigated by golden renders; there are no ladder tuning tables, because the shipped ladder implements neither tier of docs/ENGINEERING.md 2.3), and pattern-layer scope creep (mitigated by declared non-goals).
