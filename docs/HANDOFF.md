...
**Arduino Library Manager: accepted.** The mirror is
[virgilvox/bellows-embedded](https://github.com/virgilvox/bellows-embedded),
generated by `tools/build-mirror.sh`, tagged `0.1.0`, and the submission PR
(arduino/library-registry#8954) was merged by the bot with "Your submission has
now been accepted". It appears in Library Manager within a day of that. The
indexer log is at
`downloads.arduino.cc/libraries/logs/github.com/virgilvox/bellows-embedded/`,
which is the thing to read if a future version does not show up.

`.github/workflows/mirror-embedded.yml` pushes the mirror on tag. It reads
`version=` from `library.properties` rather than the git tag, because the
embedded library versions independently of the npm package (0.1.0 against
0.1.8), and it skips when the mirror already has that tag. **It needs a
`MIRROR_TOKEN` secret**, a fine-grained PAT with contents write on the mirror
...
failed on every single example: `fatal error: bellows/platform/teensy.h: No
such file or directory`, sixteen of seventeen, on the first line that mattered.
Two causes, both now fixed, both in the commit that closed this. Arduino
attributes a sketch to a library by matching includes and cannot see a nested
path, so nothing pulled the library in until every example gained
`#include <Bellows.h>`, the header `includes=` names and no example used. And
the cross-folder includes this entry used to describe as something the IDE "may
not" resolve: it does not, measured, and the mirror flattens them.

The standing lesson is the one this repository keeps relearning. That failure
was reachable by one command for as long as the port has existed, and the
document said "untested" rather than "broken" because nobody had run it.
