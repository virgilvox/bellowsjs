...
| `node tools/check-docs.mjs --check` | every flash and RAM figure a sketch produces, in `docs/HARDWARE.md`, the embedded `README.md` and the two figures rule 12 below rests on in this file: 122 of them | six stale rows in HARDWARE on its first run; then 10 stale README rows and 3 stale prose figures when it was widened. Still does NOT cover the whole-firmware Teensy table, the Daisy table, the ns tables or the board capacity table |
