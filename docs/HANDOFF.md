...
- DOCS section: fourteen tutorial pages under apps/workbench/src/docs/pages/, path-routed at /docs/<slug> (App.vue routes on pathname; the static host catchall serves the app for any path). Content accuracy contract: run pure-library snippets in Node against dist before editing claims; llm.txt is the signature source. Sitemap carries every doc URL.
- Legato: string and tube voices accept setParam('freq', hz) glides on the sounding voice (glide and legatoScratch params); the instrument store does mono legato routing when instState.legato is on. Loop playback of legato takes re-attacks (documented limitation at the note tap).
- Bowed string realism history: docs/BOWED-STRINGS.md holds both research rounds with measured evidence; the spectral gates in test/engines-physical/waveguide.test.ts are the contract. Do not loosen a gate to pass a change.
